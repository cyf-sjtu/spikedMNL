function [r,g, demand] = evaluate_sample_path_gradient_theft_group_attractiveness(numClass,numFlight,numChannel,numEpoch,rate,v,w,v0,fare,flight)
...
%         pRpy(:,:,t) = pRpy(:,:,t+1);
        for f=1:numFlight
           pRpx(f,t) = sum(sum(repmat(fare,1,numFlight).*squeeze(pupx(f,:,:,t))))...
            ...
            for j=1:numClass
                pRpy(j,f,t) =sum(sum(repmat(fare,1,numFlight).*squeeze(pupy(j,f,:,:,t))))...
                    -sum(squeeze(pupy(j,f,:,:,t)))*pRpx(:,t+1) ...
                    + pRpy(j,f,t+1);
            end
        end
    end
